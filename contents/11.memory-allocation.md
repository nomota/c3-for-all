...
The default (heap) allocator is mapped to `libc`'s memory allocation. These macros are available, just like in C.
...
Temporary allocator is designed to handle temporary memory allocation, and it removes the necessity of `free()` at all.
...
Unlike `defer` which is confined within a scope, temporary memory can go beyond a single scope `{ ... }`, but it needs to be placed within `@pool()` macro scope, in order to get properly cleaned up.

The `@pool()` macro, on exit, automatically cleans up all temporary allocations within the scope.
...
@pool()
{
   List{int} list; // Implicitly initialize with the temp allocator
   list.push(1); 
   list.push(42);
...
Stack memory is a lot faster than heap memory, and stack memories are freed automatically. So it would be very effecient to allocate memories in stack. 
...
The only drawback of OnStack allocator is that it needs to reserve given size of chunk up-front in stack, regardless of actual allocations. This might waste of stack space if the size is too large, and may cause stack overflow and panic.
...
Type* p = mem::new_aligned(Type, {initial value});
...
When you need to add additional memory at the end of the allocation, you can use `_with_padding` variations.
...
Note that all above allocations returns simple pointer or simple slice, meaning that they don't return error optional. If error occurs, they rather go panic, and do not allow catch errors.

If you want to catch memory errors, following macros are available. It's not recommended, because underlying libc's malloc() itself is not stable on memory error situation, for example, due to memory over-commit in linux. Use these when you are absolutely sure about what you're doing.
...
Type*? p = allocator::new_try(allocx, Type, {initial value});
...
Type[]? p = allocator::@tclone_slice_
try(allocx, Type[] slice);
...
* LazyTemp Allocator: it initiates reservation, only if actual allocation gets called 
* Null Allocator: if you want to make sure that no allocation gets invoked in certain area
...
* BackedArena allocator: if you want to give some specific kind of memory to Arena allocator
* DynamicArena allocator: if you want to allow the reserved chunk to grow dynamically if needed in an Arena allocator
* OnStack allocator: if you want to allocate on stack
* SimpleHeap allocatord a typical heap allocator

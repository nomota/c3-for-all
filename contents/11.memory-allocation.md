...
* Note: `@stack_pool(size)` scope may cover multiple nested `{}` scopes within it. This is similar to `@pool()` scope. We can use `@stack_pool(size)` exactly same way for `mem` allocator (without `free()`), just like we use `@pool()` for `tmem` allocator (without `free()`).  So it is possible to remove `free()` completely from our source code, without fear of memory leaks.

...
Stack memory is a lot faster than heap memory, and stack memories are freed automatically. So it would be very effective to allocate memories in stack. 
...
The only drawback of OnStack allocator is that it needs to reserve given size of chunk upfront in stack, regardless of actual allocations. This might waste of stack space if the size is too large, and may cause stack overflow error.

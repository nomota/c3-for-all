...
The `_try()` macros are indeed not that useful when using `mem`, but they might be useful if you're using another allocator, like the standard `ArenaAllocator` which will return a fault (OOM) when it has no memory left in its arena.
...
* DynamicArena allocator: if you want to allow the reserved chunk to grow dynamically when needed, in an Arena allocator. The `DynamicArenaAllocator` is essentially a linked list of `ArenaAllocator`s.

...
* Note: `@stack_pool(size)` may cover multiple nested scopes within it. This is similar to `@pool()` scope. We can use `@scope_pool(size)` exactly same way for `mem` allocator, just like we use `@pool()` for `tmem` allocator.  So it's possible to remove `free()` completely from our source code, without fear of memory leaks.

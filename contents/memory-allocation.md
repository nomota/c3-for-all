...
{
    int[MAX_NUMBER] arr; // stack allocated memory
    for (int i = 0; i < n; i++)
    {
        arr[i] = i;
    }
...
Aside from the problem with having a `MAX_NUMBER`, we can't return a pointer to this array, even as a slice, because the `arr` content gets destroyed when `create_array()` returns.
...
To use temporary allocator, there are dedicated functions/macros as follows, all with 't'-prefixed.
...
Temporary memory does not get free'd by itself. We need to confine them within certain scope, using special macros which do the necessary cleanups.
...
Without any explicit `@pool()` macro, temporary memories still get free'd at the end of `main()`.
...
@pool()
{
   ...
   io::printn(s);
}; // s and list are free'd here, because they used temp memory
```
...
Allocator allocx = &osa; // cast to common interface
```
...
fn void main()
{
    ...
    @pool() {
        test(&tra);
    }; // in fact, p gets free'd here, but tra does not know that
...
* `@stack_pool(size)`
* `@stack_mem(size; allocx)` // semicolon!, not comma
...
fn void test1() 
{
    ...
    }; // all memories get free'd here (stack popped)
...
* Note: A `@stack_pool(size)` scope may cover multiple nested `{}` scopes within it. This is similar to `@pool()` scope. We can use `@stack_pool(size)` exactly same way for `mem` allocator (without `free()`), just like we use `@pool()` for `tmem` allocator (without `free()`).  So it is possible to remove `free()` completely from our source code, without fear of memory leaks.
...
* Note: Make sure that `_aligned` memory get free'd only by `free_aligned()`
...
* BackedArena allocator: if you want to give some specific kind of memory to Arena allocator, the BackedArena allocator also falls back to its backing allocator and supports mark and sweep

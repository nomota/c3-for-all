...
* `<type>.kindof`: underlying `TypeKind` as defined in std::core::types.
...
* `<type>.typeid`: typeid for the given type. `alias`s will return the typeid of the underlying type. The typeid size is the same as that of an `iptr`.

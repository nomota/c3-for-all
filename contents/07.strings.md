...
fn Char32 u8to32(String rune_char)
{
    Char32[1] c32;
...
fn void test_iterator()
{
    ...
    assert(rune_chars[0] == u8to32("A")); // 1 byte, ASCII alphabet
    assert(rune_chars[1] == u8to32("한")); // 3 bytes, Korean alphabet
    assert(rune_chars[2] == u8to32("漢")); // 3 bytes, Chinese ideogram
    assert(rune_chars[3] == u8to32("♡")); // 3 bytes, special icon char
    assert(rune_chars[4] == u8to32("Å")); // 2 bytes, European accented char
    assert(rune_chars[5] == u8to32("😂")); // 4 bytes, Emoticon
}
```

...
* Note: string litrals are directly assignable to `String`, `char[]`, and `char*`. 
...
<* @require rune_char.len <= 4 : "Input must be 1 rune char, max 4 bytes" *>
fn Char32 c8to32(String rune_char)
{
    Char32[1] c32;
    usz? n = conv::utf8to32(rune_char, &c32);
    if (try n) {
        assert(n == 1); // must be 1 char
    }
    return c32[0];
}

fn void test_iterator()
{
    String str = "A한漢♡Å😂";
...
usz? conv::utf32to8(Char32[] utf32, char[] utf8_buffer);
...
{
    Char16[1] c16;
    usz? n = conv::utf8to16(rune_char, &c16);
    return c16[0];
}

fn String tu32to8(Char32[] c32s)
{
    char[] u8 = talloc(char, conv::utf8len_for_utf32(c32s)); // temporary memory alloc
    conv::utf32to8(c32s, u8);
    return (String)u8;
}

fn String tu16to8(Char16[] c16s)
{
    char[] u8 = talloc(char, conv::utf8len_for_utf16(c16s)); // temporary memory alloc
    conv::utf16to8(c16s, u8);
    return (String)u8;
}

fn Char16[] tu8to16(String u8)
{
    usz n = conc::utf16len_for_utf8(u8);   
    Char16* c16sp = talloc(Char16, n);  // temporary memory alloc
    conv::utf8to16_unsafe(u8, c16sp);
    return c16sp[0..n-1];
}

fn Char32[] tu8to32(String u8)
{
    usz n = conc::utf32len_for_utf8(u8);   
    Char32* c32sp = talloc(Char32, n);  // temporary memory alloc
    conv::utf8to32_unsafe(u8, c32sp);
    return c32sp[0..n-1];
}

fn void test_print() => @pool() // frees temporary memories
{
    Char16 c16 = c8to16("🇸🇪"); // 8byte national flag char
    Char32 c32 = c8to32("🔥"); // 4 byte emoji char

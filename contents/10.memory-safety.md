...
C3's default allocator is mapped into `malloc()` in `libc`. There's a `@builtin` variable that is designated to the default allocator  `mem` and its type is 'Allocator` interface in `std::core::mem::allocator` module.

Following is the most traditional way of allocating memory. `malloc()` and `free()` are `@builtin` macro.

```c3
char* p = malloc(N_BYTES);
char[] q = (char[N_BYTES])malloc(N_BYTES);
```

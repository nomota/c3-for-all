...
In combination with following things each respective, we can effectively prevent or spot the notorious vulnerability of memory leaks, without much of trouble.
...
A typical `defer` usage is like this. Putting `defer free()` right after the allocation is a good habit to prevent leaks.
...
} // deferred free(p) gets invoked here
```
...
There are many insecure C library functions, largely due to the fact that a pointer doesn't know the size of the memory that it points to.

...
This type of vulnerability can also be spotted by using `--sanitize=address` flag.
...
- Default allocator, named as `mem`
- Temporary operator, named as `tmem`
- Tracking allocator

In combination with following three things each, we can effectively prevent or spot the notorious vulnerability of memory leaks, without much of trouble.
